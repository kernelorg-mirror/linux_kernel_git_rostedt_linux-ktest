...
# Compares a good config to a bad config and tries to merge half of
# the good config with the half config and places the result in the
# result-config. After testing result-config, run the same command
# but append "good" or "bad" depending on the result.
...
# If "good" or "bad" is not stated on the command line, it will copy
# the good and bad configs to the .tmp version. If that .tmp version
# already exists, it will warn before writing over them.
# If the result-config is labeled, good, then it will copy it to the
# new good .tmp version. If the result-config is bad, it will copy
# it to the bad .tmp version. It will continue this until it can not
# merge the two any more without the result being equal to either the
# good or bad config.
...
sub config_bisect {
    my ($good_config, $bad_config, $start) = @_;
    my $ret;
...
my $ret = config_bisect $good, $bad, $start;
